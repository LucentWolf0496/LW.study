// 8*8亚马逊棋，由LEAF所作
// version2.9，悔棋+多存档+基于评估函数的单层搜索+精美棋盘【+4个难度+魔法模式（难度2）+函数代码综合优化】【制作中内容】
...
void singleTurnBot();// 函数11：正常/挑战模式-bot-单轮对局
void randomBot();// 函数12：简单模式-bot-单轮对局
void masterHuman();// 函数13：挑战模式-人类-单轮对局
bool inMap(int x , int y);// 函数14：inMap判断是否在地图内
vector<Move> canMove(int color , const vector<vector<int>> &table);// 函数15：canMove遍历求解所有可动的位置
int randomNum();// 函数16：生成1-10000随机整数
double evaluate(const vector<vector<int>> &table , int botColor , int round);// 函数17：评估函数中控模块
int QueenMove(const vector<vector<int>> &table , int botColor);// 函数18：QueenMove模块
int KingMove(const vector<vector<int>> &table , int botColor);// 函数19：KingMove模块
int position(const vector<vector<int>> &table , int botColor);// 函数20：位置价值模块
void printGrid();// 函数21：打印美观的棋盘

...
// 函数5：存盘
void fileIn(){
    ofstream fout("preserved.txt");// 相对地址，便于移植，但不会显示在Amazons文件夹
    for (int i = 0 ; i < 8 ; i ++){
        for (int j = 0 ; j < 8 ; j ++){
            fout << gridInfo[i][j] << " ";
        }
    ...
    fout << color << endl;
    fout << botColor << endl;
    fout << gameRound << endl;// 存盘存储棋盘、黑白、轮次后就可以随时调用
    fout.close();
...
// 函数6：读盘
void fileOut(){
    ifstream fin("preserved.txt");// 相对地址，便于移植，但不会显示在Amazons文件夹
    for (int i = 0 ; i < 8 ; i ++){
        for (int j = 0 ; j < 8 ; j ++){
            fin >> gridInfo[i][j];
        }
...
// 函数7：菜单
void menu(){
    char n;// **要用char，如果用int的话输入char会卡出无限循环bug
...
void singleTurnHuman(){
    ...
    while(!chessCheck){
        char x0 , y0 , x1 , y1 , x2 , y2;// char变量类型防止错误输入导致无限循环bug
        Move move;

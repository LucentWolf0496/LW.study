// 8*8亚马逊棋，由LEAF所作
// 基本要求：菜单选择+精美字符棋盘+人机对战+棋盘变化+菜单暂停+存盘读盘
// 额外功能：单步悔棋+多存档+难度选择+魔法模式+计时挑战模式
// version3.0算法更新：minimax算法+alpha-beta剪枝，提高bot强度/综合函数增效
#include <iostream>
#include <fstream>
#include <string>
#include <vector>
...
double ABSearch(vector<vector<int>> &table, int depth, 
    double alpha, double beta, bool isMaximizing, int currentRound);// 函数25：未加排序的回溯式alpha-beta剪枝
...
void singleTurnBot() {
    ...
    gridInfo[bestMove.x0][bestMove.y0] = 0;// 正式执行这一步

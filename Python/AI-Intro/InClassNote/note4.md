...
辨析高等的UCB和普通的ε-greedy的区别：ε-greedy 用 显式随机化 来探索，UCT 用 不确定性量化（上界）隐式驱动探索。
核心区别：
ε-greedy的随机性显式包含在动作选择规则中（以固定概率随机探索：选Q函数最大还是随机选择）。
UCT 的随机性仅来自模拟过程的随机结果，其动作选择公式本身是确定性的（选 UCB 最大值），没有额外掷硬币。
...
每个相关节点的总对局次数都会加1，如果最后获胜了每个相关节点都会将胜利次数加1

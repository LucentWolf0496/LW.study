...
def b2(scores:dict):
    ...
    ret = None
    temp = []
    for name , grade in scores.items():# 直接名字遍历key，.value()遍历value，.item()遍历键值对
        if (grade > 50):
            temp.append(name)
    ret = sorted(temp)# 直接这么写就可以进行默认排序
    return ret        
...
def q3(name_score_dict):
    ...
    class Student:
        def __init__(self , name , score):
            ...
            self.score = score
    res = []
    for id , grade in name_score_dict.items():
        temp = Student(id , grade)
        res.append(temp)
    n = len(res)
    for i in range (1 , n):
...
def q4(a,b):
    ...
    '''
    res = np.maximum(a, b)# 直接使用np自带的简单函数
    return res
...
def q5():
    ...
    np.random.seed(42)
    count = 0
    arr = np.random.rand(2, 100000)
    for i in range (100000):
        if arr[0][i] ** 2 + arr[1][i] ** 2 <= 1:
            count += 1
    res = 4 * count / 100000
    return res
        ...
        for i in range (x):
            ...
            for j in range (y):
                ...
                if pos == self.pacman_pos:
                    temp.append('P')
                    pacman = 1
                if walls == 0 and pacman == 0 and foods == 0:
                    temp.append(' ')
            strx = ''.join(temp)
            result.append(strx)
        return '\n'.join(result)
